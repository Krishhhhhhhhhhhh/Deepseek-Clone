...
const copyMessage=()=>{
  ...
  return (
    <div className='flex flex-col items-center w-full max-w-3xl text-[15px]'>
        <div className={`flex flex-col w-full mb-8 ${role==='user' && 'items-end'}`}>
            <div className={`group relative flex max-w-2xl py-3 rounded-xl ${role==='user'?'bg-[#414158] px-5':'gap-3' }`}>
                    ...
                    <div className='flex items-center gap-2 opacity-70'>
                        {
                            role=='user'?(
                                <>
                            <Image onClick={copyMessage} src={assets.copy_icon} alt='' className='w-4 h-auto cursor-pointer'/>
                             <Image src={assets.pencil_icon} alt='' className='w-4.5 h-auto cursor-pointer'/>
                                </>
                            ):(
                                <>
                                <Image onClick={copyMessage} src={assets.copy_icon} alt='' className='w-4.5 h-auto cursor-pointer'/>
                                <Image src={assets.regenerate_icon} alt='' className='w-4 h-auto cursor-pointer'/>
                ...
                {
                    role=='user'?(
                        <span className='text-white/90 text-[17px]'>{content}</span>
                    ):(
                        <>
